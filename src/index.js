module.exports = function check(str, bracketsConfig) {
    ...
    for (let i = 0; i < str.length; i++) {
        const type = checkType(str[i], params),
            index = (type) ? params.open.findIndex(item => item === str[i]) : params.close.findIndex(item => item === str[i]),
            lastCounter = counter[counter.length - 1];

        if (counter.length === 0 || str[i] != lastCounter[0] && type) {
      ...
      if (params.open[index] === params.close[index] && str[i] === lastCounter[0]) {
        ...
        if (!type && lastCounter[0] === params.open[index]) {
    ...
    function checkType(element, config) {
